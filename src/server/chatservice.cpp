...
ChatService::ChatService()
{
    ...
    _msgHandlerMap.insert({LOGINOUT_MSG, std::bind(&ChatService::loginout, this, _1, _2, _3)}); // 用户登出事务
    ...
    _msgHandlerMap.insert({ADD_FRIEND_MSG, std::bind(&ChatService::addFriend, this, _1, _2, _3)}); // 添加好友事务
...
void ChatService::login(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    if(user.getId() == id && user.getPwd() == pwd)
    {
        if(user.getState() == "online")
        {
            ...
            response["errno"] = 0;
            response["errmsg"] = "该账号已登录，请切换其他账号";
...
void ChatService::createGroup(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    // 存储新创建的群组信息，通过传引用，createGroup内部会设置group的id
